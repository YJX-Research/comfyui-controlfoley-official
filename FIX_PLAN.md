...
### F3.13 mux 输出的容器时长比实际内容长 ~33ms,播放器显示与 wav 不一致(用户反馈,计划外新增)
- **问题**:上游 `encode_media_with_sound` 把每帧 pts 精确钉在源 fps 网格,但从不设包时长;ffmpeg 封装器对最后一个视频包按默认 ~1/24s 猜时长(实测末包 duration=5000 ticks@1/120000,其余均 1001),容器时长被虚增(实例:内容 6.0644s → 容器 6.0977s)。前端播放器显示容器时长,于是同一次生成的 mp4 与 wav(6.0604s)显示的总长在取整边界上不一致。
- **修法**(节点侧,不动上游):Muxer 写完文件后做一次流拷贝 remux(`_fix_mux_container_timing`,不重编码),按相邻 pts 差补齐各包时长、末包用典型帧间隔。失败时打日志并保留原文件。
- **验证**:对实际输出实测:容器 6.0977 → 6.0836(视频轨 6.0644=真实跨度);727 帧全部可解码、音频采样数不变。残余 23ms 为 AAC 编码帧对齐(+1024 采样),属编解码规范行为,秒级与 0.1s 级显示已一致。
- **风险**:低。纯容器级操作,码流不动。
...
### F3.1c(70199a6,服务器实测后改判)
- **背景**:F3.1b 曾按复审意见改为"网络优先、异常回退本地",前提是网络要么通、要么快速失败。Linux 服务器实测出现第三种情况:小请求正常、大文件在约 35KB 处卡死且不抛异常,`huggingface_hub` 内部持续重试,离线回退分支永不触发——本地缓存完整也无法启动,只能靠外部设 `HF_HUB_OFFLINE=1` 绕过。
- **修法**:改回本地探测优先(`local_files_only=True` 命中即跳过网络),未命中再走正常下载。语义已实测确认:未缓存仓库抛 `LocalEntryNotFoundError`,缓存完整则秒过、零网络访问。报错文案补充 `HF_HUB_OFFLINE` 出路。
- **代价**(明确接受):残缺缓存不再自动补全,后果由"启动即报缺权重"承担,报错文案已给出三条出路。相较之下,静默挂起是实测发生的真实故障,残缺缓存是理论情形。
- **复审意见**:指出未缓存/残缺路径仍可能在网络层挂起,建议加 wall-clock 超时。**拒绝**:HF 下载为同步阻塞调用,外包超时需另起线程且无法中止底层传输,可能留下半成品缓存,比现状更糟;网络不稳属部署环境问题,不由节点包用超时猜测。列为已知残留。
